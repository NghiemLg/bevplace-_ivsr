...
def train_epoch(epoch, model, train_set):
    ...
    if epoch>=0:
        ...
        with h5py.File(train_set.cache, mode='w') as h5: 
            pool_size = model.global_feat_dim  # 8192 for ViT
    ...
    with h5py.File(initcache, mode='w') as h5: 
        with torch.no_grad():
            ...
            for iteration, (query, _, _, _) in enumerate(data_loader, 1):
                ...
                # ViT local features có shape [B, 768, 14, 14]
                local_feat = local_feat.view(query.size(0), 768, -1).permute(0, 2, 1)  # [B, 196, 768]
        ...
        print('====> Clustering..')
        niter = 100
        kmeans = faiss.Kmeans(768, 64, niter=niter, verbose=False)  # Changed from 128 to 768 for ViT
...
if __name__ == "__main__":
    ...
    model = REIN_ViT(input_dim=768, output_dim=8192, rotations=8)
    model = model.cuda()
    ...
    if opt.load_from != '':
        ...
        # Không cần khởi tạo NetVLAD cho ViT model
        print('===> ViT model initialized without NetVLAD')
        model = model.cuda()
    ...
    if opt.mode.lower() == 'train':
        ...
        # Check if checkpoint exists for test mode
        if opt.load_from == '':
            print("Error: Test mode requires a checkpoint. Please specify --load_from path.")
            exit(1)
