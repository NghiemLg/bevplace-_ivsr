...
def getClusters(cluster_set):
    ...
    sampler = SubsetRandomSampler(np.random.choice(len(cluster_set), n_im, replace=False).tolist())
...
if __name__ == "__main__":
    ...
    if opt.load_from != '':
        ...
        with h5py.File(initcache, mode='r') as h5: 
            clsts_data = h5.get("centroids")
            traindescs_data = h5.get("descriptors")
            if clsts_data is not None and traindescs_data is not None:
                clsts = np.array(clsts_data)
                traindescs = np.array(traindescs_data)
                model.pooling.init_params(clsts, traindescs)
            else:
                raise RuntimeError("Centroids or descriptors not found in HDF5 file.")
            model = model.cuda()
    ...
    if opt.mode.lower() == 'train':
        ...
        logdir = None
        if writer.file_writer is not None:
        ...
        for epoch in range(opt.nEpochs):
            ...
            for seq in ['00', '02', '05', '06','08']:
                ...
                if isinstance(recall_top1, float):
        ...
        for seq in eval_seq:   
            if seq=='08':
                test_set = kitti_dataset.InferDataset(seq=seq,sample_inteval=50)  #return a very large local feature mat could be very slow. sample the dataset to reduce ram and time cost
