...
# Lớp linear để biến đổi class embedding
class ClassEmbeddingProjector(nn.Module):
    def __init__(self, input_dim=768, output_dim=8192):
        super(ClassEmbeddingProjector, self).__init__()
        self.linear = nn.Linear(input_dim, output_dim)
    
    def forward(self, x):
        # x có shape [B, 768] (class embedding)
        return self.linear(x)  # Output shape [B, 8192]

# Khởi tạo projector
class_embedding_projector = ClassEmbeddingProjector(input_dim=768, output_dim=8192)
