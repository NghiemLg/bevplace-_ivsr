...
def evaluateResults(seq, global_descs, local_feats, dataset, match_results_save_path=None):
    ...
    faiss_index.add(np.ascontiguousarray(global_descs[:dataset.db_split_index], dtype=np.float32))

    _, predictions = faiss_index.search(np.ascontiguousarray(global_descs[dataset.db_split_index+int(200/dataset.sample_inteval):], dtype=np.float32), 1)  #top1
    ...
    for q_idx, pred in enumerate(predictions):
        ...
        if len(positives)>0:
            ...
            if match_results_save_path is not None:
                ...
                points1 = np.array([query_kps[m.queryIdx].pt for m in all_match], dtype=np.float32) 
                points2 = np.array([db_kps[m.trainIdx].pt for m in all_match], dtype=np.float32)

                # Dòng 56: Đảm bảo truyền đủ tham số cho hàm (nếu là np.float32, truyền vào giá trị cụ thể)
                # Dòng 58: Đảm bảo truyền đủ tham số cho hàm (nếu là cv2.kmeans, truyền đủ k, distances, labels)
                # Dòng 104-107: Chuyển list sang numpy array trước khi thực hiện phép toán
                points1 = np.array(points1, dtype=np.float32)
                points2 = np.array(points2, dtype=np.float32)
                H, mask, max_csc_num = rigidRansac((np.array([[im_side//2,im_side//2]], dtype=np.float32)-points1)*0.4,(np.array([[im_side//2,im_side//2]], dtype=np.float32)-points2)*0.4)
                
                q_pose = dataset.poses[query_idx]

                q_pose = np.array(q_pose[:12], dtype=np.float32).reshape(3,4)
                q_pose = np.hstack((q_pose[:2,:2], q_pose[:2,3].reshape(-1,1)))
                q_pose = np.vstack((q_pose, np.array([[0,0,1]], dtype=np.float32)))

                db_pose = dataset.poses[index]
                db_pose = np.array(db_pose[:12], dtype=np.float32).reshape(3,4)
                db_pose = np.hstack((db_pose[:2,:2], db_pose[:2,3].reshape(-1,1)))
                db_pose = np.vstack((db_pose, np.array([[0,0,1]], dtype=np.float32)))
