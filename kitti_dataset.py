    ...
    for q_idx, pred in enumerate(predictions):
        ...
        if len(positives)>0:
            ...
            if match_results_save_path is not None:
                ...
                # Map keypoint coordinates from original image (201x201) to feature space (224x224)
                scale_factor = 224.0 / 201.0
                query_kps_mapped = []
                db_kps_mapped = []
                
                for kp in query_kps:
                    x, y = kp.pt
                    x_mapped = int(x * scale_factor)
                    y_mapped = int(y * scale_factor)
                    x_mapped = min(max(x_mapped, 0), 223)
                    y_mapped = min(max(y_mapped, 0), 223)
                    query_kps_mapped.append((x_mapped, y_mapped))
                ...
                for kp in db_kps:
                    ...
                    x_mapped = min(max(x_mapped, 0), 223)
                    y_mapped = min(max(y_mapped, 0), 223)
                    db_kps_mapped.append((x_mapped, y_mapped))
                
                query_des = [local_feats[query_idx][y, x] for x, y in query_kps_mapped]
                db_des = [local_feats[index][y, x] for x, y in db_kps_mapped]
